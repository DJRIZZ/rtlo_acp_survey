# code to create data summaries and figures based on 2023 data from the acp loon helicopter survey
# Dan Rizzolo
# 2023-03-21
...
data <- read.csv("data/data_2023/2023_ACP_loon_data.csv")

data$count <-as.numeric(data$count)
data$nest <- as.numeric(data$nest)
data$pair <- as.numeric(data$pair)
data$Nvisit <- as.character(data$visit)
data$Nplot <- as.character(data$plot)
...
# In 2023 sampling design was dependent double observer
# Seat Left Front (LF) the primary observer recording all count, nest, and pair observations
# Seat Left Rear (LR) is the secondary observer recording count, nest, and pair observations NOT MADE by the primary (RF) observer

# summarize data to get total count across observers by visit and species
# aggregate across seats by plot and visit for each endpoint (count, nest, pair)
#sum_plot_vis_sp_ct <- aggregate(data$count ~ data$plot*data$visit*data$species, FUN = "sum")
#colnames(sum_plot_vis_sp_ct) <- c("plot", "visit", "species", "count")

# what was the total number of loons seen across plots during first visit
#sum_visit_count <- aggregate(sum_plot_visit_count$count ~ sum_plot_visit_count$visit , FUN = "sum")
#sum_visit_count-sp <- aggregate(sum_plot_visit_count$count ~ sum_plot_visit_count$visit  , FUN = "sum")

# what was the total number of loons seen across plots during first visit by species
...
data_wide <- cbind.data.frame(species = data$species,
                              plot = data$plot,
                              visit = data$visit,
                              seat = data$seat,
                              bird = data$count,
...
# aggregate across seats (primary and secondary observers) to get complete count
